...
# 🧠 ADAPTIVE META AI v3
# ------------------------------------------------------------
# A data-driven ensemble that combines multiple independent
# signals, detects unstable/high-entropy regimes, calibrates
# confidence from walk-forward history, and can return WAIT.
# This is intentionally numpy-only; it does not pretend to be
# a trained LSTM/GRU when no neural model has been fitted.
# ============================================================

def _history_from_docs(history_docs):
    docs = list(reversed(history_docs or []))
    return [d.get("size") for d in docs if d.get("size") in ("BIG", "SMALL")]


def _binary_features(hist, window):
    seg = hist[-window:] if len(hist) >= window else hist
    if not seg:
        return np.array([0.5, 0.5, 0.0, 0.0, 0.0], dtype=float)
    b = np.array([1.0 if x == "BIG" else 0.0 for x in seg], dtype=float)
    p = float(b.mean())
    last = b[-1]
    changes = float(np.mean(b[1:] != b[:-1])) if len(b) > 1 else 0.0
    # signed momentum, recent half minus older half
    half = max(1, len(b)//2)
    mom = float(b[-half:].mean() - b[:-half].mean()) if len(b) > 1 and len(b[:-half]) else 0.0
    return np.array([p, last, changes, mom, float(len(seg))], dtype=float)


def _transition_prob(hist, order=2, alpha=1.0):
    if len(hist) <= order:
        return 0.5
    key = tuple(hist[-order:])
    big = small = alpha
    for i in range(order, len(hist)):
        if tuple(hist[i-order:i]) == key:
            if hist[i] == "BIG": big += 1
            else: small += 1
    return big / (big + small)


def _pattern_similarity_prob(hist, max_order=5, alpha=1.0):
    # Finds historical contexts matching the current suffix.
    probs = []
    weights = []
    for order in range(1, max_order + 1):
        ...
        for i in range(order, len(hist)):
            if tuple(hist[i-order:i]) == key:
                # recent occurrences count slightly more
                recency = 1.0 + i / max(1, len(hist))
                if hist[i] == "BIG": big += recency
                else: small += recency
        total = big + small
        probs.append(big / total)
        weights.append(min(2.0, 0.5 + order * 0.25))
    if not probs:
        return 0.5
    return float(np.average(probs, weights=weights))


def _signal_probabilities(hist):
    if not hist:
        return {}
    ps = {}
    for w in (4, 8, 16, 32, 64):
        if len(hist) >= 2:
            ps[f"freq{w}"] = _binary_features(hist, w)[0]
    for order in (1, 2, 3, 4):
        ps[f"markov{order}"] = _transition_prob(hist, order)
    ps["pattern"] = _pattern_similarity_prob(hist, 5)
    # EMA probabilities on several horizons
    for span in (5, 10, 20):
        ps[f"ema{span}"] = _ema_ratio(hist, span)
    # Contrarian streak signal, deliberately kept near neutral.
    side, streak = _streak(hist)
    if side:
        ps["streak"] = (0.5 + min(streak, 5) * 0.035) if side == "BIG" else (0.5 - min(streak, 5) * 0.035)
    return ps


def _walk_forward_signal_scores(hist, min_train=80, max_points=500):
    """Estimate per-signal hit rates without peeking at future results."""
    if len(hist) < min_train + 20:
        return {}, 0
    start = max(min_train, len(hist) - max_points)
    names = list(_signal_probabilities(hist[:start]).keys())
    scores = {n: [1.0, 1.0] for n in names}  # successes, failures
    for t in range(start, len(hist)):
        past = hist[:t]
        actual = 1.0 if hist[t] == "BIG" else 0.0
        signals = _signal_probabilities(past)
        for n, prob in signals.items():
            if n not in scores:
                scores[n] = [1.0, 1.0]
            if (prob >= 0.5) == (actual >= 0.5):
                ...
                scores[n][1] += 1.0
    rates = {n: a/(a+b) for n, (a,b) in scores.items() if a+b >= 20}
    return rates, max(0, len(hist)-start)


def _calibrate_probability(raw_p, hist):
    """Conservative calibration using out-of-sample directional accuracy."""
    if len(hist) < 120:
        return 0.5 + (raw_p - 0.5) * 0.65
    rates, _ = _walk_forward_signal_scores(hist, min_train=80, max_points=250)
    if not rates:
        return 0.5 + (raw_p - 0.5) * 0.7
    # Shrink extreme probabilities unless historical evidence supports them.
    evidence = float(np.mean([abs(r - 0.5) for r in rates.values()]))
    shrink = min(0.85, max(0.45, 0.65 + evidence))
    return float(np.clip(0.5 + (raw_p - 0.5) * shrink, 0.02, 0.98))


def adaptive_meta_ai_predict(history_docs, model_accuracies=None):
    """Adaptive ensemble: signal quality + regime + calibration + WAIT."""
    hist = _history_from_docs(history_docs)
    if len(hist) < 40:
        return "wait", "🧠 Adaptive Meta AI → WAIT", 0.0, "Need >= 40 completed results"

    signals = _signal_probabilities(hist)
    rates, tested = _walk_forward_signal_scores(hist, min_train=80, max_points=400)

    # Weight signals by out-of-sample edge, with a neutral floor.
    weighted = 0.0
    weight_sum = 0.0
    contributions = []
    for name, p in signals.items():
        rate = rates.get(name, 0.5)
        # Edge is measured only against 50%; no fabricated confidence.
        w = 0.25 + min(1.5, abs(rate - 0.5) * 8.0)
        if model_accuracies and name in model_accuracies:
            w *= 0.75 + 0.75 * float(np.clip(model_accuracies[name], 0.0, 1.0))
        weighted += p * w
        weight_sum += w
        contributions.append((name, p, rate, w))

    raw = weighted / weight_sum if weight_sum else 0.5
    calibrated = _calibrate_probability(raw, hist)

    # Regime: high entropy + weak edge means less confidence, not a fake boost.
    recent = hist[-32:]
    ent = _entropy(recent)
    changes = np.mean(np.array(recent[1:]) != np.array(recent[:-1])) if len(recent) > 1 else 0.5
    max_edge = max([abs(r - 0.5) for r in rates.values()] or [0.0])
    separation = abs(calibrated - 0.5) * 2.0
    regime_penalty = 0.0
    if ent > 0.98:
        regime_penalty += 0.18
    if changes > 0.65:
    ...
    if max_edge < 0.025:
        regime_penalty += 0.12
    effective = separation * max(0.0, 1.0 - regime_penalty)

    # Evidence-aware WAIT. Thresholds are deliberately conservative.
    if tested < 60 or effective < 0.12:
        return "wait", "🧠 Adaptive Meta AI → WAIT", effective * 100, \
               f"Regime unstable / evidence weak | entropy={ent:.2f} tested={tested}"

    pred = "BIG" if calibrated >= 0.5 else "SMALL"
    conf = 50.0 + effective * 50.0
    conf = float(np.clip(conf, 50.0, 88.0))
    burmese, dot = _label(pred)
    top = sorted(contributions, key=lambda x: x[3], reverse=True)[:3]
    top_txt = ", ".join(f"{n}:{r*100:.1f}%" for n,_,r,_ in top)
